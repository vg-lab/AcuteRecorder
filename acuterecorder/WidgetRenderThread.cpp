 ...
 * @param ts the TimeStamp marking the startButton of this thread.
 */
inline void printDebugVelocity( int i , TimeStamp ts )
{
  if ( i % 100 == 0 )
  {
    const auto delay = std::chrono::high_resolution_clock::now( ) - ts;
    const float secs = std::chrono::duration_cast< std::chrono::milliseconds >(
      delay ).count( );
